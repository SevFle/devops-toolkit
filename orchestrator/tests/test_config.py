...
class TestConfigDefaults:
    def test_default_values(self):
        ...
        assert config.max_implementation_attempts == 25
        assert config.max_review_cycles == 23
...
class TestConfigFromEnv:
    ...
    def test_partial_env_uses_defaults(self):
        ...
        assert config.max_review_cycles == 23  # default
...
class TestConfigValidation:
    ...
    def test_invalid_attempts(self):
        ...
        assert any("max_implementation_attempts" in error for error in errors)

    def test_invalid_review_cycles(self):
        config = Config(max_review_cycles=0)
        errors = config.validate()
        assert any("max_review_cycles" in error for error in errors)

    def test_invalid_timeout(self):
        config = Config(claude_timeout=10)
        errors = config.validate()
        assert any("claude_timeout" in error for error in errors)

    def test_invalid_review_timeout(self):
        config = Config(review_timeout=5)
        errors = config.validate()
        assert any("review_timeout" in error for error in errors)

    def test_invalid_time_budget_seconds(self):
        config = Config(time_budget_seconds=0)
        errors = config.validate()
        assert any("time_budget_seconds" in error for error in errors)

    def test_invalid_max_consecutive_no_progress(self):
        config = Config(max_consecutive_no_progress=0)
        errors = config.validate()
        assert any("max_consecutive_no_progress" in error for error in errors)
